...
function criaMain($tipo) {
    if ($tipo == 1) { //cria o início do main
        return '<main>
            ...
            <br>
              ...
              <tbody';
    } else if ($tipo == 2) { //cria o final do main
        return '  
        </tbody>  
        </table>
...
function imprimeMensagem () {
    ...
    if ($cnpj == "existe"){
        return "<h4 class='erro'>Erro! Não foi possível editar pois sse CNPJ já está cadastrado!</h4><br><br>";
    }
} 

$fornecedorControl = new FornecedorControl($db);
$resultados = $fornecedorControl->listarObj(); //chama a função responsável por buscar os valores no banco de dados e transformar em objetos
